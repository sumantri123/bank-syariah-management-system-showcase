...
$('button#btn_search').on('click', function () {                          
    ...
    $('#search').focus();        
    $("#id_rek").select2().select2('val','""');
    $("#id_kode_tran").select2().select2('val','""');                    
    $("#id_rek").select2({ width: "100%" });    
    $("#id_kode_tran").select2({ width: "100%" });           
    //totalDebetKredit();
...
function addRow(){
    ...
    var content = "<label for='inputCity' class='form-label' style='color:blue; font-weight:bold'>Transaksi</label>"
        ...
        content += "<th width='10%' style='font-size:12px' class='text-center'><b>Kd</b></th>"
...
$("#search").keypress(function (e) {
    ...
    if(e.keyCode==13){
        ...
        $.ajax({
            type: 'post',
            url: "" + base_url + "/search/tranPenTun",
            dataType: 'JSON',
            data: {
                _token: CSRF_TOKEN,
                bagian: bagian,
            ...
            success: function (data) {
                if (data.status == 'oke') {
                    ...
                    $("#id_rek").val(data.jbIdRekening).trigger('change');
                    $("#id_rek").select2({ width: "100%" });          
                    $("#id_kode_tran").val(data.jbKodeTransaksi).trigger('change');                    
                    $('#kode_transaksi').val(data.jbKodeTransaksi2);                                                                                                                 
                    ...
                    content += "<td class='qty' style='font-size:12px'>"+data.jbKodeTransaksi2+"</td>"
                    content += "<td class='qty' style='font-size:12px'>"+convertToRupiahNoRp(kredit)+"</td>"
                    content += "</tr>";
...
function getval(sel){                
	...
	$('#id_perkiraan1').val( $("#id_rek option:selected").attr("perkiraan"));  	                
    $('#id_tab').val($("#id_rek option:selected").attr("norek"));  
	$('#no_rekening').val($("#id_rek option:selected").attr("no_rekening"));  
   /* if(selected !=""){
        $.ajax({
            type: 'GET',
...
function getval2(sel){                
    ...
    if(selected !=""){
        $.ajax({
            type: 'GET',
            url: "" + base_url + "/GetKodeTransPen/" + selected,
            dataType: 'JSON',            
            success: function (data) {
                if (data.status == 'oke') {                    
                    $('#kode_transaksi').val(data.kodeTransaksi); 
                    $('#keterangan').val(data.namaTransaksi);               
                } else if (data.status == 'null') {
...
function deleteProses(id) {        
    ...
    $.ajax({
        type: 'GET',
        url: "" + base_url + "/delete/tranPenTun/" + id,
        dataType: 'JSON',        
        beforeSend: function(){
            BeforeSend();
        },
...
function insertUpdateInduk() {
    ...
    if (form.valid() == true) {
        ...
        var method = $('#method_field').val();                    
        var action_url = "" + base_url + "/tranPenTun";  
        var action_type = "Tambah";                      
        if (method === "PUT") {
            action_url = "" + base_url + "/tranPenTun/" + $('#idTr').val();
            action_type = "Ubah";
        }
        ...
        $.ajax({
            ...
            success: function (data) {                    
                if (data.status == 'insert_successful') {                        
                    ...
                    content += "<td class='unit' style='font-size:12px'>"+data.jbNoRekening+"</td>"              
                    content += "<td class='text-left' style='font-size:12px'>"+data.jbTgl+"</td>"                                        
                    content += "<td class='qty' style='font-size:12px'>"+data.jbKodeTransaksi+"</td>"
                    content += "<td class='qty' style='font-size:12px'>"+convertToRupiahNoRp(kredit)+"</td>"
                    content += "</tr>";
