...
function loadData(dateCari){   
    ...
    $.ajax({
        type: 'POST',
        url: "" + base_url + "/getDataLapNasPiMd/lapNP",        
        dataType: 'JSON',
        data: {
            _token: CSRF_TOKEN,
            //search: request.term
            ...
            if (data.status == 'oke') { 
                ...
                if(data.total >0){
                    ...
                    var content = "<div style='overflow-x:auto;'>"
                        ...
                        content += "<th width='20%' class='text-center'><b>Nomor Rekening</b></th>"
                        content += "<th width='35%' class='text-center'><b>Nama Nasabah</b></th>"
                        content += "<th width='20%' class='text-center'><b>Saldo Pinjaman</b></th>"
//                        content += "<th width='20%' class='text-center'><b>Bagi Hasil</b></th>"
                        content += "</tr>"
                        content += "</thead>"
                        content += "<tbody>"                    
                    ...
                    for (b = 0; b < totDetData; b++) {                         
                        ...
                        // }
                        //$nominal*($sukuBunga/100/12)*$jangkaWaktu,
                        var bungaEfektifAnuitas = parseInt(data.data[b].bunga_efektif_anuitas)/100/12;
                        var jangkaWaktu = data.data[b].jangka_waktu;
						var marginDitangguhkan = (data.data[b].nominal_efektif_anuitas===null)?0:parseInt(data.data[b].nominal_efektif_anuitas);
                        //var saldoBungaAwal = data.data[b].nominal_pokok * bungaEfektifAnuitas * jangkaWaktu;
                        var nominalPokok = parseInt(data.data[b].nominal_pokok);						
                        var totAngsuran = (data.data[b].kredit_angsuran===null)?0:parseInt(data.data[b].kredit_angsuran);
                        var totBungaEfektif = (data.data[b].kredit_suku_bunga===null)?0:parseInt(data.data[b].kredit_suku_bunga);
						//alert(nominalPokok +"-"+ marginDitangguhkan +"-("+ totAngsuran +"-"+ data.data[b].debit_suku_bunga);
                        var saldoPinjamanAkhir = nominalPokok - marginDitangguhkan - totAngsuran;
                        var saldoBungaAkhir = marginDitangguhkan - totBungaEfektif;
                        //var saldoBungaAkhir = saldoBungaAwal;
                        ...
                        content += "<td class='text-left'>"+data.data[b].nomor_rekening+"</td>"                         
                        content += "<td class='text-left'>"+(data.data[b].nama)+"</td>"                         
                        content += "<td><span style='float: right;'>"+convertToRupiahNoRp(saldoPinjamanAkhir)+"</span></td>"
//                        content += "<td><span style='float: right;'>"+convertToRupiahNoRp(saldoBungaAkhir)+"</span></td>"
                        content += "</tr>";  
                        totalSaldoPinjaman += parseInt(saldoPinjamanAkhir);                                          
                        totalSaldoBunga += parseInt(saldoBungaAkhir);                                          
                    ...
                    content += "<td><b><span style='float: right;'>"+convertToRupiahNoRp(totalSaldoPinjaman)+"</span></b></td>"                    
//                    content += "<td><b><span style='float: right;'>"+convertToRupiahNoRp(totalSaldoBunga)+"</span></b></td>"                    
                    content += "</tr>"          
                    content += "</tfoot>"
                    content += "</table>"
                    content += "</div>";
