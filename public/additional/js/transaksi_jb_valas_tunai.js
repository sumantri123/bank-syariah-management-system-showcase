...
//	alert(text_select);
$('#keterangan').val(text_select);
/*
    if(selected === "2.1"){
        $('#keterangan').val("Penjualan Bank Notes");  	                
    } else if (selected === "2.2"){
        $('#keterangan').val("Pembelian Bank Notes");  	                
    } else if (selected === "3.1"){
        $('#keterangan').val("Penjualan TC");  	                
    } else if (selected === "3.2"){
        $('#keterangan').val("Pembelian TC");  	                
    } else {
        $('#keterangan').val("");  	                
    }*/
}
...
$("#search").keypress(function (e) {
    if(e.keyCode==13){
        ...
        $.ajax({
            type: 'post',
            url: "" + base_url + "/search/jurnalBagianDetValas",
            dataType: 'JSON',
            data: {
                _token: CSRF_TOKEN,
                bagian: bagian,
            ...
            success: function (data) {
                if (data.status == 'oke') {
                    ...
                    for (b = 0; b < totDetData; b++) {
                        ...
                        var debet = ((data.data[b].id_jenis_transaksi)=="1") ? data.data[b].jurnal_det_nominal_valas :"";
                        var kredit = ((data.data[b].id_jenis_transaksi)=="2") ? data.data[b].jurnal_det_nominal_valas :"";
...
function totalDebetKredit(){
    var id = document.getElementById("id_jb").value;
    var PendapatanValas = document.getElementById("pendapatan_valas_id").value;
    if(id!=""){
        $.ajax({
            type: 'GET',
            url: "" + base_url + "/total/jurnalBagianDetValas/" + id,
            dataType: 'JSON',
            success: function (data) {
                if (data.status == 'oke') {                    
                    ...
                    if(message == "Balance"){                       
                            ...
                            content += "<tr class='body' id='row_3'>"
                            content += "<td class='unit'>"
                            content += "<input type='text' class='form-control form-control-sm' id='kode_perkiraan_3' value='405.003' name='kode_perkiraan[]' readonly>"
                            content += "</td>"
                            content += "<td class='text-left'>"
                            content += "<input type='text' class='form-control form-control-sm' id='nama_perkiraan_3' value='PENDAPATAN DARI TRANSAKSI VALUTA ASING' name='nama_perkiraan[]' readonly>"
                            content += "<input type='hidden' class='form-control form-control-sm' id='id_perkiraan_3' value='"+PendapatanValas+"' name='id_perkiraan[]' readonly>"                            
                            content += "</td>"
                            content += "<td class='unit'><input type='text' class='form-control form-control-sm' style='text-align:right' id='debet_3' value='0' name='debet[]' readonly></td>"
                            content += "<td class='qty'><input type='text' class='form-control form-control-sm' style='text-align:right' id='kredit_3' value='0' name='kredit[]' readonly></td>"
                            content += "<td class='unit'>"                            
                            content += "</td>"
                            content += "</tr>";
    ...
    document.onkeydown = function(){
        if(window.event && window.event.keyCode == 113) {                                
            ...
            if(countRow < 2) {
                ...
                info_noti('Silahkan Membuat Transaksi Baru');
            }
        }
    }
...
function deleteProses(id) {    
    ...
    $.ajax({
        type: 'GET',
        url: "" + base_url + "/delete/jurnalBagianValas/" + id,
        dataType: 'JSON',        
        beforeSend: function(){
            BeforeSend();
        },
...
function insertUpdateInduk() {
    ...
    if (form.valid() == true) {
        ...
        var method = $('#method_field').val();                    
        var action_url = "" + base_url + "/jurnalValasTunai";  
        var action_type = "Tambah";                      
        if (method === "PUT") {
            action_url = "" + base_url + "/jurnalValasTunai/" + $('#id_jb').val();
            action_type = "Ubah";
        }
...
function saveDet(b){
    ...
    if(idJurnalBagian==""){
        ...
        var action_url = "" + base_url + "/jurnalBagianDetValas";  
        var action_type = "Tambah";        
        ...
        $.ajax({
            ...
            data: {
                ...
                idJB:idJurnalBagian, 
                transaksi:transaksi, 
                kodePer:kodePerkiraan, 
                idPer:idPerkiraan, 
                idTrans:idJenisTransaksi, 
                nominal:jurnalDetNominal
            },            
            beforeSend: function(){
                BeforeSend();
            },
...
function delDet(b){
    ...
    if(idJurnalBagianDet==""){        
        ...
        info_noti('Silahkan Menghapus No. Induk, Pada Button Hapus Diatas');
        // Lobibox.confirm({
        //     iconClass: true,
        //     title: 'Delete Data',                        
...
//     $.ajax({
//         type: 'GET',
//         url: "" + base_url + "/delete/jurnalBagianDetValas/" + id+"/" + idJB,
//         dataType: 'JSON',
//         success: function (data) {
//             if (data.status == 'delete_successful') {
//                 success_noti('Data Berhasil Terhapus');
