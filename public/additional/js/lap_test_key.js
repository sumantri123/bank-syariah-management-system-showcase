...
function loadData(dateCari){   
    ...
    $.ajax({
        type: 'POST',
        url: "" + base_url + "/getDataLapTestKey/lapTestKey",        
        dataType: 'JSON',
        data: {
            _token: CSRF_TOKEN,
            //search: request.term
            ...
            if (data.status == 'oke') { 
                ...
                if(data.total >0){
                    ...
                    var content = "<div style='overflow-x:auto;'>"
                        ...
                        content += "<th width='15%' class='text-center'><b>No Telex</b></th>"                                                
                        content += "<th width='20%' class='text-center'><b>Pengirim</b></th>"
                        content += "<th width='20%' class='text-center'><b>Penerima</b></th>"                        
                        content += "<th width='20%' class='text-center'><b>Nominal</b></th>"                        
                        content += "<th width='10%' class='text-center'><b>Test Key</b></th>"                        
                        content += "<th width='10%' class='text-center'><b>Validasi</b></th>"                        
                        content += "</tr>"
                        content += "</thead>"
                        content += "<tbody>"                    
                    ...
                    for (b = 0; b < totDetData; b++) {                                                 
                        ...
                        content += "<td class='text-left'>"+data.data[b].test_key_no+"</td>"                         
                        content += "<td class='text-left'>"+data.data[b].nama_pengirim+"</td>"                         
                        content += "<td class='text-left'>"+data.data[b].nama_penerima+"</td>"                         
                        content += "<td><span style='float: right;'>"+convertToRupiahNoRp(data.data[b].nominal)+"</span></td>"
                        content += "<td class='text-left'>"+data.data[b].test_key+"</td>"
                        content += "<td class='text-left'>"+data.data[b].hasil_test_key+"</td>"
                        content += "</tr>";                          
